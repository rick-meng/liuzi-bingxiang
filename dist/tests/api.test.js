...
const app = createApp();
function makeExpiry(daysFromNow, timezone) {
    return DateTime.now()
        .setZone(timezone)
...
async function addPantryItem(market, payload) {
...
describe("NZ/AU pantry MVP APIs", () => {
    ...
    it("supports NZ recommendation and shopping list with correct currency/unit behavior", async () => {
        ...
        const doubanjiang = shoppingResp.body.items.find((item) => item.ingredientKey === "doubanjiang");
    ...
    it("returns AU-specific substitutions and packaging suggestions", async () => {
        ...
        expect(recipeResp.body.aiAssist.substitutionHints.some((line) => line.includes("dry sherry"))).toBe(true);
    ...
    it("prioritizes expiry-first recipes when coverage is similar", async () => {
        ...
        expect(recResp.status).toBe(200);
        const expiryRecipe = recResp.body.expiryFirst.find((item) => item.recipeId === "r_egg_tomato");
        const fullRecipe = recResp.body.full.find((item) => item.recipeId === "r_bokchoy_garlic");
    ...
    it("filters out recipes that exceed max cook minutes", async () => {
        ...
        expect(allResults.some((item) => item.recipeId === "r_potato_beef")).toBe(false);
        expect(allResults.every((item) => item.recipe.cookMinutes <= 20)).toBe(true);
    });
    it("triggers expiry alerts by local timezone rules for NZ and AU", async () => {
        ...
        const nzAlert = await request(app).get("/alerts/expiry?userId=u_alert_nz&market=NZ&timezone=Pacific/Auckland");
        const auAlert = await request(app).get("/alerts/expiry?userId=u_alert_au&market=AU&timezone=Australia/Sydney");
    ...
    it("supports fixed profile update and dynamic settings persistence", async () => {
        ...
        expect(schemaResp.body.fields.some((field) => field.key === "spiceLevel")).toBe(true);
