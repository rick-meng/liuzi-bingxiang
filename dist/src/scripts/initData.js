...
async function writeGeneratedFile(filename, payload) {
...
async function main() {
    ...
    const report = {
        ...
        notes: "Catalog built from NZ supermarket sitemap signals + NZ asian supermarket departments, then constrained recipe generation and quality gate."
    };
    ...
    console.log(`Initialized data: ${catalog.length} ingredients, ${quality.accepted.length} generated recipes, ${FOUNDATION_RECIPES.length} foundation recipes.`);
}
main().catch((error) => {
    // eslint-disable-next-line no-console
    console.error(error);
