...
export interface StoreSourceItem {
  source: string;
  url: string;
  signalType: "product_slug" | "category_slug" | "recipe_slug" | "department";
  tokens: string[];
  error?: string;
}
...
export interface CollectedStoreSignals {
...
function parseTaiPingDepartments(html: string): string[] {
...
async function collectPakNSaveSignals(
  maxProductSitemaps: number,
  maxProductsPerSitemap: number
): Promise<StoreSourceItem[]> {
  const items: StoreSourceItem[] = [];
...
async function collectNewWorldSignals(): Promise<StoreSourceItem[]> {
...
async function collectTaiPingSignals(): Promise<StoreSourceItem[]> {
