...
const SUBSTITUTION_RULES: Record<
  string,
  { ingredientKey: string; nameZh: string; nameEn: string; note: string }[]
> = {
  shaoxing_wine: [
    {
...
function groupByCategory(catalog: IngredientCatalogItem[]) {
  return {
...
function chooseUnit(candidateUnits: Unit[], preferred: Unit[]): Unit {
  const match = preferred.find((unit) => candidateUnits.includes(unit));
  return match ?? candidateUnits[0] ?? "g";
}

function quantityFor(item: IngredientCatalogItem, unit: Unit): number {
  if (item.category === "protein") {
    if (unit === "g") return 260;
    if (unit === "piece") return item.ingredientKey === "egg" ? 2 : 1;
    return 1;
  }
...
function toRecipeId(seed: string): string {
  const digest = createHash("sha1").update(seed).digest("hex").slice(0, 10);
  return `g_${digest}`;
}

function buildSubstitutions(ingredients: RecipeIngredient[]): RecipeSubstitution[] {
...
function buildTags(ingredients: RecipeIngredient[], cookMinutes: number): string[] {
  ...
  if (
    ingredients.some((item) => item.ingredientKey === "tofu_firm") &&
...
function buildStirFryRecipe(
  protein: IngredientCatalogItem,
  vegetable: IngredientCatalogItem,
  aromatics: IngredientCatalogItem[],
  sauces: IngredientCatalogItem[]
): Recipe {
  ...
  ].filter((item): item is RecipeIngredient => item !== undefined);
...
function buildRiceBowlRecipe(
  protein: IngredientCatalogItem,
  vegetable: IngredientCatalogItem,
  staples: IngredientCatalogItem[],
  sauces: IngredientCatalogItem[]
): Recipe | null {
...
function buildTofuRecipe(
  vegetables: IngredientCatalogItem[],
  sauces: IngredientCatalogItem[]
): Recipe | null {
...
export function generateConstrainedRecipes(
  catalog: IngredientCatalogItem[],
  overrideOptions?: Partial<GenerationOptions>
): Recipe[] {
  ...
  for (const protein of stirFryProteins) {
    for (const vegetable of stirFryVeggies) {
      ...
      const bowlRecipe = buildRiceBowlRecipe(
        protein,
        vegetable,
        grouped.staples,
        grouped.sauces
      ...
      const noodleRecipe = buildNoodleRecipe(
        protein,
        vegetable,
        grouped.staples,
        grouped.sauces
